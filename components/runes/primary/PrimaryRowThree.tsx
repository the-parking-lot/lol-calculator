...
const PrimaryRowThree = ( { setPrimaryRowThree, children, rune, lesserRune, runesInfo, index }: RuneProps ) => {
    ...
    <>
      <Tooltip label={runeInfo} aria-label="tooltip" bg={useColorModeValue('gray.200', 'gray.700')} textColor={useColorModeValue('black', 'white')}>
        <IconButton aria-label={`${children}`}
          ...
          mx={3}
          mb={10}
          onClick={() => setPrimaryRowThree(children)}
        />
      </Tooltip>
    </>
  )
...
export default PrimaryRowThree;